...
def campbell_lidf(
    ...
    *,
    ...
    if batched:
        ...
        # Case 2: excent > 1 (planophile / oblate ellipsoid)
        ...
        # Case 3: excent < 1 (erectophile / prolate ellipsoid)
    ...
    # Scalar path (unchanged from the original implementation)
    excent_s = excent
    freq = torch.zeros(n_elements, dtype=dtype, device=device)
    for idx in range(n_elements):
        tl1s = tl1[idx]
        tl2s = tl2[idx]
        x1 = excent_s / torch.sqrt(1.0 + excent_s**2 * torch.tan(tl1s) ** 2)
        x2 = excent_s / torch.sqrt(1.0 + excent_s**2 * torch.tan(tl2s) ** 2)
        if torch.isclose(excent_s, torch.tensor(1.0, dtype=dtype, device=device), atol=1e-12):
            freq[idx] = torch.abs(torch.cos(tl1s) - torch.cos(tl2s))
        else:
            alph = excent_s / torch.sqrt(torch.abs(1.0 - excent_s**2))
            alph2 = alph**2
            x1_sq = x1**2
            x2_sq = x2**2
            if excent_s > 1:
